ChatSignaturePlus = LibStub("AceAddon-3.0"):NewAddon("ChatSignaturePlus", "AceConsole-3.0", "AceHook-3.0", "AceEvent-3.0")

local L = LibStub("AceLocale-3.0"):GetLocale("ChatSignaturePlus", true)
...
function ChatSignaturePlus:Migration()
    if(ChatSignaturePlusSettings and ChatSignaturePlusSettings.Version) then
        local need_new_profile = false
        
        if(self.db.profile.identity ~= "") then
            if(self.db.profile.enabled ~= ChatSignaturePlusSettings.Enabled) then need_new_profile = true end
            if(self.db.profile.format ~= ChatSignaturePlusSettings.Format) then need_new_profile = true end
            if(self.db.profile.identity ~= ChatSignaturePlusSettings.MainName) then need_new_profile = true end
            if(self.db.profile.fun ~= ChatSignaturePlusSettings.Fun) then need_new_profile = true end
            
            for key, value in pairs(ChatSignaturePlusSettings.Channels) do
                if(key == "Guild") then 
                    if(self.db.profile.channels["GUILD"].enabled ~= value) then need_new_profile = true end
                elseif(key == "Officer") then 
                    if(self.db.profile.channels["OFFICER"].enabled ~= value) then need_new_profile = true end
                elseif(key == "Tell") then 
                    if(self.db.profile.channels["WHISPER"].enabled ~= value) then need_new_profile = true end
                elseif(key == "Instance") then
                    if(self.db.profile.channels["INSTANCE_CHAT"].enabled ~= value) then need_new_profile = true end
                    if(self.db.profile.channels["INSTANCE_CHAT"].identity ~= ChatSignaturePlusSettings.NickName) then need_new_profile = true end
                elseif(key == "Raid") then
                    if(self.db.profile.channels["RAID"].enabled ~= value) then need_new_profile = true end
                    if(self.db.profile.channels["RAID"].identity ~= ChatSignaturePlusSettings.NickName) then need_new_profile = true end
                elseif(key == "Party") then
                    if(self.db.profile.channels["PARTY"].enabled ~= value) then need_new_profile = true end
                    if(self.db.profile.channels["PARTY"].identity ~= ChatSignaturePlusSettings.NickName) then need_new_profile = true end
                else
                    ...
                    if(name) then
                        if(self.db.profile.channels.customs[name] and self.db.profile.channels.customs[name].enabled ~= value) then 
            ...
            if(need_new_profile) then
                local profile_name = ChatSignaturePlusSettings.Version .. " - " .. UnitName("player") .. "-" .. GetRealmName()
                
                ChatSignaturePlus.db:SetProfile(profile_name)
                
                self:Print(L["migration.created_new_profile"](profile_name))
            end
        end
        
        self.db.profile.enabled = ChatSignaturePlusSettings.Enabled
        self.db.profile.format = ChatSignaturePlusSettings.Format
        self.db.profile.identity = ChatSignaturePlusSettings.MainName
        self.db.profile.fun = ChatSignaturePlusSettings.Fun
        
        for key, value in pairs(ChatSignaturePlusSettings.Channels) do
            if(key == "Guild") then 
                self.db.profile.channels["GUILD"].enabled = value
            elseif(key == "Officer") then 
                self.db.profile.channels["OFFICER"].enabled = value
            elseif(key == "Tell") then 
                self.db.profile.channels["WHISPER"].enabled = value
            elseif(key == "Instance") then
                self.db.profile.channels["INSTANCE_CHAT"].enabled = value
                self.db.profile.channels["INSTANCE_CHAT"].identity = ChatSignaturePlusSettings.NickName
            elseif(key == "Raid") then
                self.db.profile.channels["RAID"].enabled = value
                self.db.profile.channels["RAID"].identity = ChatSignaturePlusSettings.NickName
            elseif(key == "Party") then
                self.db.profile.channels["PARTY"].enabled = value
                self.db.profile.channels["PARTY"].identity = ChatSignaturePlusSettings.NickName
            else
                local s, e, number = string.find(key, "C(%d%d)")
        ...
        ChatSignaturePlusSettings = {}
    
        self:Print(L["migration.finished"](need_new_profile))
    end
end

function ChatSignaturePlus:OnInitialize()
    self.db = LibStub("AceDB-3.0"):New("ChatSignaturePlusDB", defaults, true)
    
    if(ChatSignaturePlusSettings) then
        self:Migration()
    end
    
    options = {
        name = L["options.name"] or "ChatSignaturePlus",
        handler = self,
        type = "group",
        childGroups = "tab",
        args = {
            ...
            format = {
                ...
                set = function(info, value) 
                    if(value == "" or not string.find(value, "%%s")) then 
                        self.db.profile.format = "[%s]" 
                    else 
                        self.db.profile.format = value 
                    end 
                end,
    ...
    LibStub("AceConfig-3.0"):RegisterOptionsTable("ChatSignaturePlus", options)
    generalOptions = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("ChatSignaturePlus", "ChatSignaturePlus")

    LibStub("AceConfig-3.0"):RegisterOptionsTable("ChatSignaturePlus Profiles", LibStub("AceDBOptions-3.0"):GetOptionsTable(self.db))
    profilesOptions = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("ChatSignaturePlus Profiles", L["profiles.name"], "ChatSignaturePlus")
    
    -- neue, passende Slash-Commands
    self:RegisterChatCommand("chatsignatureplus", "SlashProcessor")
    self:RegisterChatCommand("csp", "SlashProcessor")
    ...
    function self:SlashProcessor(input)
        ...
        if Settings and Settings.OpenToCategory then
            Settings.OpenToCategory("ChatSignaturePlus")
...
function ChatSignaturePlus:RefreshConfig()
    self:LoadDefaultChannels()
    self:LoadCustomChannels()
    self:LoadCommunities()
    
    LibStub("AceConfigRegistry-3.0"):NotifyChange("ChatSignaturePlus")
end

function ChatSignaturePlus:LoadDefaultChannels()
...
function ChatSignaturePlus:addCustomChannel(info, name)
    if(name == "") then
...
function ChatSignaturePlus:removeCustomChannel(channel)
...
function ChatSignaturePlus:LoadCustomChannels()
    options.args.custom_channels.args = {
        add = {
            ...
            validate = function(info, value) 
                if(value == "") then 
                    return L["options.custom_channels.error.empty"] 
                elseif(self.db.profile.channels.customs[value]) then 
                    return L["options.custom_channels.error.already_exists"] 
                else 
                    return true 
                end 
            end
...
function ChatSignaturePlus:addCommunity(info, clubId)
...
function ChatSignaturePlus:removeCommunity(community)
...
function ChatSignaturePlus:LoadCommunities()
...
function ChatSignaturePlus:FunTime(identity, mode)
    local value
...
function ChatSignaturePlus:IsDayForFun()
    local havingFun = "NO"
    local dtime = date("*t")
...
function ChatSignaturePlus:AlterMessage(msg, channel)
    if(channel) then
        if(channel.enabled) then
            local identity = channel.identity
            
            if(identity == "" or not identity) then
...
function ChatSignaturePlus:findCustomChannel(name)
...
function ChatSignaturePlus:PreviewMessage(channel)
    local identity = channel.identity
    ...
    if(chatInfo.colorNameByClass) then
        ...
        if classInfo and RAID_CLASS_COLORS[classInfo.classFile] then
			classColor = RAID_CLASS_COLORS[classInfo.classFile].colorStr;
		else
            classColor = "FF" .. (classColors[classID] or "FFFFFF")
        end
