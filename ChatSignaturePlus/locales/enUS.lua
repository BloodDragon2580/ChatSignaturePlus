local L = LibStub("AceLocale-3.0"):NewLocale("ChatSignaturePlus", "enUS", true)
...
if L then
    ...
    L["migration.finished"] = function(created_new_profile)
        local text = "Migration of this character's configurations finished"
        if(created_new_profile) then
            text = text .. ". A new profile was created because of conflicts with the active profile"
        end
        return text
    end
    ...
    L["options.enable.desc"]       = "Enable/Disable Chat Signature Plus"
    L["options.fun.name"]          = "Fun"
    L["options.fun.desc"]          = "Let Chat Signature Plus know if you like to have fun on special days"
